...
mod tests {
    ...
    /// Command 9 runs until a (0,0) pair.
    #[test]
    fn multiple_displacements_stop_at_the_zero_pair() {
        let mut g = HashMap::new();
        g.insert(1u16, vec![0x01, 0x09, 0x05, 0x00, 0x05, 0x00, 0x00, 0x00, 0x0A, 0x02, 0x00]);
        let o = interp(&g).run(1);
        // Two segments drawn, then the terminator, then an octant arc that
        // must not be mistaken for more displacement pairs.
        assert_eq!(o.strokes[0].len(), 3, "got {:?}", o.strokes);
    }
