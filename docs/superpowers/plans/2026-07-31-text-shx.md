...
- **Offline builds:** `ttf-parser 0.25.1` and `subsetter 0.2.6` are already in the local cargo cache (`~/.cargo/registry/cache/`). Use `cargo build --offline` / `cargo test --offline`. Do **not** gate on `cargo fetch --offline`: it fails on this project for a pre-existing, unrelated reason — `wayland-protocols`, a Linux-only transitive dependency of `eframe`, is absent from the cache — and that failure says nothing about whether the crate you added resolves. Verified by reproducing it against an unmodified `Cargo.toml`.
...
- Test: unit tests in `src/text/ttf.rs`. These read real Windows system fonts, so they are golden tests already — no separate golden file is needed for this task.
...
mod tests {
    ...
    /// text is full of it. Every Latin SHX font carries it at its own code
    /// point (measured: simplex 81, isocp 97, txt 80, gbenor 96, complex
    /// 80 glyphs in `0x80..0xFF`, all five with both `°` and `±`), and
    /// `gbcbig` carries it at its GBK code `0xA1E3`. A routing rule that
    /// reaches neither drops it silently.
    #[test]
...
impl FontPair {
    ...
    /// drawing the degree sign encodes to two bytes (`0xA1E3`) and lives
    /// in `gbcbig.shx`, while `simplex.shx` carries the same character at
    /// its own code point `0xB0`. A `0x80` boundary sends it to the big
    /// font under the wrong key, reaches neither font, and silently drops
    /// it — along with `±` and the 80-97 other glyphs every Latin SHX font
    /// carries in `0x80..0xFF`. Those are exactly what `%%d` and `%%p`
    /// produce, so it would erase the degree symbol from every dimension.
    ///
    ...
    pub fn char_geom(&mut self, ch: char, cp: Codepage) -> Option<CharGeom> {
        if bigfont_code(ch, cp).is_some() {
            if let Some(found) = self.bigfont.char_geom(ch, cp, true) {
                return Some(found);
            }
            return self.primary.char_geom(ch, cp, false);
        }
        // A space has no outline but still advances, so the primary
        // answering with empty geometry is a success, not a miss.
...
```rust
    ...
    fn centred_and_right_aligned_text_uses_the_alignment_point() {
        ...
        // 1.048 em against 0.762 em of ink — see the measured ground
        // truth above), so the visible edge stops short of the point by
        // that gap. That is the small offset drafters see when they snap
        // right-justified text to a grid line, and asserting the ink
        // lands exactly on the point instead would force the layout onto
        // ink-extent justification, which no text engine does.
    ...
    /// R-TXT-3.2, group 72 = 5 "Fit": the run is stretched or compressed
    /// to span exactly between its two points, at its natural height.
    ///
    /// Measured in the reference drawing: 227 entities use this — 12.9%
    /// of all TEXT/ATTRIB — every one with a real non-zero span, mostly
    /// single characters at height ~286 fitted into ~200-unit spans.
    /// Those are grid-bubble and axis labels; drawn at natural width they
    /// overflow their circle.
    #[test]
    fn fitted_text_is_compressed_to_span_its_two_points() {
        ...
        let fit = bounds(
            &lay_out(&text_entity("ABCDEF", &fitted), &StyleRecord::default(), &mut fonts, Codepage::Gbk)
                .unwrap(),
        );
        let natural = bounds(
            &lay_out(&text_entity("ABCDEF", &codes), &StyleRecord::default(), &mut fonts, Codepage::Gbk)
                .unwrap(),
        );
...
pub fn lay_out(
    ...
    let mut widest = lines.iter().map(|l| l.width).fold(0.0f64, f64::max);
    let line_count = lines.len() as f64;

    // 72 = 5 "Fit": AutoCAD preserves the height and scales the *width
    // factor* so the run spans exactly from the insertion point to the
    // alignment point, taking its rotation from that vector rather than
    // from group 50.
    //
    // Left unhandled this falls through to plain left alignment at
    // natural width, which is 227 entities in the reference drawing —
    // 12.9% of its TEXT/ATTRIB, every one with a real span — rendered
    // about 40% too wide. They are grid-bubble and axis labels, so the
    // symptom is a character spilling out of its circle.
    let mut fit_rotation: Option<f64> = None;
    if !is_mtext && entity.int(72, 0) == 5 {
        ...
        if span.is_finite() && span > 0.0 && natural.is_finite() && natural > 0.0 {
            ...
            fit_rotation = Some((target.y - insertion.y).atan2(target.x - insertion.x).to_degrees());
        }
    }
    ...
    let (dx, dy) = if is_mtext {
        ...
        // Fit has already been scaled to its span, so it starts at the
        // insertion point with no justification offset of its own.
        let column = if horizontal == 5 { 0.0 } else { column };
        (-widest * column / 2.0, dy)
    };

    // The origin every offset is measured from. Fit is the exception
    // among the non-zero justifications: its 11/21 is the *end* of the
    // span it was just scaled to, not the point it hangs from.
    let uses_alignment_point = if is_mtext {
        false
    } else {
        entity.int(72, 0) != 5 && (entity.int(72, 0) != 0 || entity.int(73, 0) != 0)
    };
    ...
    let transform = Affine::translation(dx, dy)
        ...
        // Fit takes its angle from the vector between its two points,
        // which overrides group 50.
        .then(Affine::rotation(fit_rotation.unwrap_or(rotation)))
